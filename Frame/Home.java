...
public class Home extends JFrame implements ActionListener,MouseListener{
	...
	JButton progress,tips,help,probutton,signout,aboutus,userplan,exit,selectSport;
	JPanel panel;
...
public Home (String name,String password){
	...
	userplan.setBounds(0,500,200,50);
	userplan.setBackground(mycolor);
	userplan.setFont(new Font("Open Sans",Font.BOLD,20));
	...
	selectSport =new JButton("Select Sports"); 
	selectSport.setBounds(0,250,200,50);
	selectSport.setBackground(mycolor);
	selectSport.setFont(new Font("Open Sans",Font.BOLD,20));
    selectSport.setCursor(new Cursor(Cursor.HAND_CURSOR));
	selectSport.setBorderPainted(false);
	selectSport.setFocusable(false);
	selectSport.addActionListener(this);
	imglabel.add(selectSport);
	
	Title=new JLabel("Sporting Sphere");//1st title line
	Title.setBounds(650,450,500,100);
	...
	STitle=new JLabel("Complex");//2nd title line
	STitle.setBounds(650,500,600,100);
	STitle.setFont(new Font("Open Sans",Font.BOLD,60));
	STitle.setForeground(new Color(0,255,51));
	imglabel.add(STitle);
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==probutton){
		...
		else if(ae.getSource()==selectSport){
		SportSelection Sp = new SportSelection(name,password);
		Sp.setVisible(true);
		this.setVisible(false);
		}
